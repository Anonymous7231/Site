	...
	public Table() {
		...
		Button EditButton = new Button("EditButton") {
			...
			public void onSubmit() {
				...
				//cryptoprotocolversion=TLSv1.2
				String connectionUrl = "jdbc:sqlserver://;servername=USER-ПК\\SQLEXPRESS;port=1433;database=MyPage;integratedSecurity=false;user=sa;password=qwerty";
				String SQL = "UPDATE Пользователи SET Login='" + EditLoginField.getDefaultModelObjectAsString() + "', Password='" + EditPasswordField.getDefaultModelObjectAsString() + "' WHERE ID=" + EditIDField.getDefaultModelObjectAsString();
				try (Connection con = DriverManager.getConnection(connectionUrl); PreparedStatement update = con.prepareStatement(SQL, Statement.RETURN_GENERATED_KEYS);) {
					...
					update.close();
					con.close();
					editfeedback.setDefaultModelObject("Пользователь успешно изменен!");
		...
		Button ConfirmButton = new Button("ConfirmButton") {
			...
			public void onSubmit() {
				...
				try (Connection con = DriverManager.getConnection(connectionUrl); Statement stmt = con.createStatement();) {
					String SQL = "SELECT COUNT(*) FROM Пользователи WHERE ID="+ EditIDField.getDefaultModelObjectAsString();
					ResultSet rs = stmt.executeQuery(SQL);
					while (rs.next()) {
						if (rs.getString(1).equals("1")) {
				            mybool = true;
				...
				if (mybool) {
					...
					editfeedback.setDefaultModelObject("Введите новый логин и пароль.");
				}
				else {
					EditLoginField.setVisible(false);
					EditPasswordField.setVisible(false);
